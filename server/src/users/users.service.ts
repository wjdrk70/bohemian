...
export class UsersService {
  ...
  async registerUser(createUserData: CreateUserInput): Promise<CreateUserOutput> {
    try {
     ...
     await this.UserRepository.save(
      ...
      return{success:true,message:`${createUserData.nickname}님 회원가입축하드립니다`};
    } catch (error) {
      Logger.log('db저장 실패!');
  ...
  async login({ email, password }: LoginInput): Promise<LoginOutput> {
    try {
      ...
      return {success:true,message:`${userEmail.nickname}님이 로그인하셨습니다`,token};
